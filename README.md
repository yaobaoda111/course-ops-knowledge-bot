...
I built this small service after watching course notes, learner dates, and educator reporting rules drift into separate documents. It took me an evening and the only infrastructure in the example is Infrai: its OpenAI-compatible `baseURL` creates embeddings, while the same key reaches vector search and reranking through one API.
